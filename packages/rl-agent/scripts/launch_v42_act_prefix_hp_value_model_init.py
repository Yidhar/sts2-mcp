#!/usr/bin/env python3
"""Launch the reviewed v42 Act-prefix / HP-loss-value lineage.

The immutable model source is v39's fully byte-verified healthy anchor at
local step 50,261. V42 changes replay, model-head, transaction-horizon, and
matcher ABIs, so this is explicit model initialization, never exact resume. A
pinned cross-process attestation reuses
the prior full payload validation and avoids repeatedly hashing 2.8 GiB of
replay sidecars; manifest, metadata, file set, sizes and model ABI remain
fail-closed.
"""
...
def _load_base() -> Any:
    ...
    module_name = "_sts2_v42_act_prefix_hp_value_model_init_base"
...
RUN_NAME = "full-run-revival-v42-act-prefix-hp-value-model-init"
...
SOURCE_ATTESTATION_RELATIVE = Path(
    "validation/checkpoint-attestations/v39-healthy-050261-byte-verified-v1.json"
)
...
CONFIG_FILE = "full_run_revival_v42_act_prefix_hp_value_model_init.toml"
LINEAGE_LABEL = "v42"

SCHEMA_VERSION = "sts2-v42-act-prefix-hp-value-model-init-preflight-v1"
STATE_SCHEMA_VERSION = "sts2-v42-act-prefix-hp-value-model-init-state-v1"
SUPERVISED_SCHEMA_VERSION = (
    "sts2-v42-act-prefix-hp-value-model-init-supervised-launch-v1"
)
SUPERVISED_STATE_SCHEMA_VERSION = (
    "sts2-v42-act-prefix-hp-value-model-init-supervised-state-v1"
)
...
def source_attestation_path(paths: Any) -> Path:
    return (paths.artifact_root / SOURCE_ATTESTATION_RELATIVE).resolve(strict=False)
...
def _verify_source_checkpoint(
    ...
    """Reuse the pinned prior full-byte proof; never re-hash replay sidecars."""

    checkpoint = _base.source_checkpoint_path(paths)
    attestation = source_attestation_path(paths)
    if not checkpoint.is_dir():
        raise _base.LaunchError(f"fixed v39 model source is missing: {checkpoint}")
    if not attestation.is_file():
        raise _base.LaunchError(
            f"fixed source-byte attestation is missing: {attestation}"
        )
    source = r"""
import hashlib
import json
