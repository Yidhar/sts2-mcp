#!/usr/bin/env python3
"""Launch the reviewed v46 factual macro-option actor lineage.

The immutable network source is v44's healthy validation checkpoint at local
environment step 100,478. V46 changes grounded encoding, transaction replay
and training semantics: exact native upgrade previews, selection-origin option
returns, symmetric rest/forge option credit, and a gated bounded actor bridge.
...
budget-40 curriculum remain unchanged. Preflight revalidates every fixed v44
...
def _load_base() -> Any:
    ...
    module_name = "_sts2_v46_macro_option_actor_model_init_base"
...
RUN_NAME = "full-run-revival-v46-macro-option-actor-model-init"
...
CONFIG_FILE = "full_run_revival_v46_macro_option_actor_model_init.toml"
LINEAGE_LABEL = "v46"

SCHEMA_VERSION = "sts2-v46-macro-option-actor-model-init-preflight-v1"
STATE_SCHEMA_VERSION = "sts2-v46-macro-option-actor-model-init-state-v1"
SUPERVISED_SCHEMA_VERSION = (
    "sts2-v46-macro-option-actor-model-init-supervised-launch-v1"
)
SUPERVISED_STATE_SCHEMA_VERSION = (
    "sts2-v46-macro-option-actor-model-init-supervised-state-v1"
)
