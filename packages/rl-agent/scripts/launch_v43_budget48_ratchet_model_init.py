#!/usr/bin/env python3
"""Launch the reviewed v43 budget-48 ratchet lineage.

The immutable model source is v42's healthy validation gate at local
environment step 100,109 — the 10/16 peak gate of the act-prefix/HP-value
course. V43 changes the revival budget 64 -> 48 (the reward-v7 unit price
crosses the per-floor progress value, making the rest-vs-forge HP trade-off
economically identifiable) and replays exploration/liveness schedules from
lineage zero. Reward-v7 semantics are unchanged but the curriculum differs,
so this is explicit model initialization, never exact resume. Full source
payload validation runs in preflight; no prior attestation is reused.
"""
...
def _load_base() -> Any:
    ...
    module_name = "_sts2_v43_budget48_ratchet_model_init_base"
...
RUN_NAME = "full-run-revival-v43-budget48-ratchet-model-init"
SOURCE_RUN_ID = "e5bf7c98-ef09-4648-91c9-4a6cb58a0082"
SOURCE_ENVIRONMENT_STEPS = 100_109
...
SOURCE_CHECKPOINT_ID = "34c8c6ef-457d-42ae-bb37-f6189e40289c"
...
SOURCE_POLICY_VERSION = 1_621
SOURCE_LEARNER_UPDATES = 1_621
SOURCE_CHECKPOINT_RELATIVE = Path(
    "checkpoints/full-run-revival-v42-act-prefix-hp-value-model-init"
...
CONFIG_FILE = "full_run_revival_v43_budget48_ratchet_model_init.toml"
LINEAGE_LABEL = "v43"

SCHEMA_VERSION = "sts2-v43-budget48-ratchet-model-init-preflight-v1"
STATE_SCHEMA_VERSION = "sts2-v43-budget48-ratchet-model-init-state-v1"
SUPERVISED_SCHEMA_VERSION = (
    "sts2-v43-budget48-ratchet-model-init-supervised-launch-v1"
)
SUPERVISED_STATE_SCHEMA_VERSION = (
    "sts2-v43-budget48-ratchet-model-init-supervised-state-v1"
)
...
def build_supervisor_command(
    ...
    *,
    ...
    """Re-enter this adapter so the immutable v42 source pins remain bound."""
