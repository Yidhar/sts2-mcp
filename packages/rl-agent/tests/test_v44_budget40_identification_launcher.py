...
SPEC = importlib.util.spec_from_file_location(
    "v44_budget40_identification_launcher",
    SCRIPT,
)
...
def test_v44_changes_only_budget_and_schedule_replay_from_v43() -> None:
    ...
    assert config.environment == v43.environment

    assert v43.curriculum.revival_budget == 48
    ...
    assert v43.runtime.model_initialization_liveness_schedule_mode == "reset"
    ...
    assert config.runtime.seed == v43.runtime.seed == 6_300_000
...
def test_v44_is_pinned_model_initialization_from_v43_healthy_gate(
    tmp_path: Path,
) -> None:
    ...
    assert launcher.SOURCE_RUN_ID == "df33b12b-5dc8-43cd-b745-a8751a8c65e6"
    assert launcher.SOURCE_ENVIRONMENT_STEPS == 100_212
    assert launcher.SOURCE_TOTAL_ENVIRONMENT_STEPS == 250_000
    assert launcher.TARGET_ENVIRONMENT_STEPS == 250_000
    assert launcher.SOURCE_POLICY_VERSION == 1_626
    assert launcher.SOURCE_LEARNER_UPDATES == 1_626
    assert launcher.SOURCE_CHECKPOINT_ID == (
        "1b309a8c-fdef-4508-bea9-f14ec0631638"
    )
    ...
    assert launcher.source_checkpoint_path(paths).name == (
        "healthy-validation-step-000100212"
    )
...
def test_v44_checkpoint_pin_rejects_every_identity_mismatch(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
...
def test_v44_supervisor_reenters_adapter_and_keeps_pins(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    manifest = tmp_path / "v44.launch.json"
    ...
    assert launcher._core.RESUME_STEP == 100_212
