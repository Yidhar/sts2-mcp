...
SCRIPT = PACKAGE_ROOT / "scripts/launch_v33_rocdxg_exact_continuation_100k.py"
CONTINUATION_CONFIG = (
    PACKAGE_ROOT
    / "config/experiments/full_run_revival_v33_rocdxg_exact_continuation_100k.toml"
)
SOURCE_CONFIG = (
    PACKAGE_ROOT
    / "config/experiments/full_run_revival_v33_rocdxg_exact_resume_smoke.toml"
)
SPEC = importlib.util.spec_from_file_location("v33_rocdxg_exact_continuation_100k", SCRIPT)
...
def test_continuation_changes_only_mutable_runtime_configuration() -> None:
    ...
    assert continuation.runtime.total_environment_steps == launcher.TARGET_ENVIRONMENT_STEPS
    assert launcher.ADDITIONAL_ENVIRONMENT_STEPS == 58_550
    assert continuation.runtime.log_dir.endswith("v33-rocdxg-exact-continuation-100k")
    assert continuation.runtime.checkpoint_dir.endswith("v33-rocdxg-exact-continuation-100k")
    assert continuation.runtime.checkpoint_interval_steps == 15_000
    assert continuation.runtime.total_environment_steps % 15_000 != 0
    assert continuation.runtime.evaluation_steps == (50_000, 75_000)
    assert continuation.runtime.early_evaluation_steps == ()
    assert continuation.runtime.final_audit_steps == (100_000,)
    assert continuation.runtime.evaluation_liveness_guard_enabled is True
    assert continuation.runtime.evaluation_guard_failure_action == "rollback_continue"


def test_continuation_command_is_exact_resume_and_environment_is_rocdxg_hermetic(
    tmp_path: Path,
) -> None:
...
def test_continuation_checkpoint_identity_and_counters_are_fully_pinned(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    summary = {
