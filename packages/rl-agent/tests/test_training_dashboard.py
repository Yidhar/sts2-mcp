...
def test_learner_projection_surfaces_fresh_policy_signal_diagnostics(tmp_path: Path) -> None:
    ...
    _append_jsonl(
        metrics,
        {
            ...
            "policy_version": 7,
            "episodic_sequences": 2,
            "episodic_success_policy_candidate_labels": 9,
            "episodic_policy_labels": 0,
            ...
            "episodic_sampling": {
                ...
                "fresh_policy_quota_missed": 1,
                "fresh_policy_candidate_episodes": 4,
                "fresh_policy_candidate_decisions": 13,
    ...
    assert parser.latest_learner is not None
    projected = parser.latest_learner
    assert projected["episodic_success_policy_candidate_labels"] == 9
    assert projected["episodic_policy_labels"] == 0
    assert projected["episodic_policy_active_sequences"] == 0
    assert projected["episodic_failure_policy_suppressed_labels"] == 3
    ...
    assert projected["fresh_policy_candidate_episodes"] == 4
    assert projected["fresh_policy_candidate_decisions"] == 13
    ...
    _append_jsonl(
        metrics,
        {
            ...
            "policy_version": 8,
            "episodic_sequences": 2,
            "episodic_success_policy_candidate_labels": 5,
            "episodic_policy_labels": 5,
            "episodic_policy_active_sequences": 1,
            "episodic_failure_policy_suppressed_labels": 0,
            "episodic_policy_lag_suppressed_labels": 0,
            "episodic_sampling": {
                "fresh_policy_quota_requested": 1,
                "fresh_policy_quota_filled": 1,
                "fresh_policy_quota_missed": 0,
                "fresh_policy_candidate_episodes": 2,
                "fresh_policy_candidate_decisions": 5,
    ...
    assert parser.latest_learner is not None
    assert parser.latest_learner["episodic_policy_active"] is True
    assert parser.latest_learner["episodic_zero_policy_label_update"] is False
...
def test_learner_projection_does_not_invent_missing_policy_signal(tmp_path: Path) -> None:
    metrics = tmp_path / "metrics.jsonl"
    _append_jsonl(
        metrics,
        _run_start(RUN_A, 1.0),
        {
            "event": "learner_update",
            "unix_s": 2.0,
            "environment_steps": 64,
    ...
    assert parser.latest_learner is not None
    assert parser.latest_learner["episodic_policy_active"] is None
    assert parser.latest_learner["episodic_zero_policy_label_update"] is None
    assert parser.latest_learner["fresh_policy_quota_requested"] is None
    assert parser.latest_learner["fresh_policy_quota_filled"] is None
    assert parser.latest_learner["fresh_policy_quota_missed"] is None
    assert parser.latest_learner["fresh_policy_candidate_episodes"] is None
    assert parser.latest_learner["fresh_policy_candidate_decisions"] is None
