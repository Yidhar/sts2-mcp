...
class DictObservationEncoder:
    ...
    def _enc_actions(
        ...
        if semantic_texts:
            semantic_embeddings = self._get_encoder().encode_batch(semantic_texts)
            for embedding_index, slot in enumerate(semantic_text_slots):
                semantic_action_text[slot] = semantic_embeddings[embedding_index]
