...
v2 按非强制决策的归一化熵检测软塌缩。当前策略熵是“分支熵 + 按分支候选数归一化
后的条件熵”，不是普通扁平候选分布熵，因此不能除以 `log(|A_legal|)`。令合法分支
`b` 的候选数大于 1 时 `q_b=1`，否则 `q_b=0`；该 count-balanced hierarchical
entropy 的精确容量是：

```text
H_capacity = log(sum_b(exp(q_b)))
H_normalized = H_policy / H_capacity
```

实现直接从与策略相同的分支 ID 和 mask 计算该容量，并只对最终无量纲诊断做 `[0,1]`
数值规范化。这样既保留原熵目标的“候选数量不偏置分支”性质，也保证断路器阈值有稳定
语义。首个 v37 启动正是在第一次 learner update 上由旧的错误分母 fail-closed；该
run 未完成任何参数更新，已废弃且不得续训。

断路器至少要求 16 个有效决策：平均归一化熵不高于 0.10，且至少 75% 的决策不高于
该阈值，连续 8 个 learner batch 后，将熵权重临时抬至至少 0.012，共 16 次更新。
